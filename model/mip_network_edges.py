...
from utils.data_utils import sparse_func, InputDataHolder, sparse_dense_mul_1d, MIPBatchHolder
...
class MIPNetwork(torch.nn.Module):
    # TODO: This is poor implementation and most likely doesn't work
    ...
    def __init__(self, output_bits, feature_maps=64, pass_steps=3, summary=None):
        ...
        self.edge_update = nn.Sequential(
            nn.Linear(self.feature_maps * 3 + 4, self.feature_maps),
            PairNorm(subtract_mean=False),
            nn.ReLU(),
            nn.Linear(self.feature_maps, self.feature_maps),
        )

        self.eq_edge_update = nn.Sequential(
            nn.Linear(self.feature_maps * 3 + 4, self.feature_maps),
            PairNorm(subtract_mean=False),
            nn.ReLU(),
            nn.Linear(self.feature_maps, self.feature_maps),
    ...
    def forward(self, batch_holder: MIPBatchHolder, device):
        ...
        _, eq_const_count = batch_holder.vars_eq_const_graph.size()
        _, objective_count = batch_holder.vars_inst_graph.size()
        _, edge_count = batch_holder.vars_edge_graph.size()
        _, eq_edge_count = batch_holder.eq_vars_edge_graph.size()
        ...
        edge_values = torch.unsqueeze(batch_holder.edge_values, dim=-1)
        eq_edge_values = torch.unsqueeze(batch_holder.eq_edge_values, dim=-1)
        ...
        edges = torch.ones([edge_count, self.feature_maps], device=device)
        eq_edges = torch.ones([eq_edge_count, self.feature_maps], device=device)

        for i in range(self.pass_steps):
            var_input = torch.cat([variables, relaxed_solution, int_mask], dim=-1)
            const_input = torch.cat([constraints, const_values], dim=-1)
            eq_const_input = torch.cat([eq_constraints, eq_const_values], dim=-1)

            var_edges = var_input[batch_holder.vars_edge_graph.indices()[0, :]]
            const_edges = const_input[batch_holder.const_edge_graph.indices()[0, :]]
            e2c = torch.cat([var_edges, edges, edge_values, const_edges], dim=-1)
            e2c = self.e2c_mlp(e2c)
            e2c_msg = torch.sparse.mm(batch_holder.const_edge_graph, e2c)
            e2v_msg = torch.sparse.mm(batch_holder.vars_edge_graph, e2c)

            eq_var_edges = var_input[batch_holder.eq_vars_edge_graph.indices()[0, :]]
            eq_const_edges = eq_const_input[batch_holder.eq_const_edge_graph.indices()[0, :]]

            e2c_eq = torch.cat([eq_var_edges, eq_edges, eq_edge_values, eq_const_edges], dim=-1)
            e2c_eq = self.e2c_eq_mlp(e2c_eq)
            e2c_eq_msg = torch.sparse.mm(batch_holder.eq_const_edge_graph, e2c_eq)
            e2v_eq_msg = torch.sparse.mm(batch_holder.eq_vars_edge_graph, e2c_eq)

            variables = self.variable_update(torch.cat([e2v_msg, e2v_eq_msg, variables], dim=-1))
            constraints = self.constraint_update(torch.cat([e2c_msg, constraints], dim=-1))
            eq_constraints = self.eq_constraint_update(torch.cat([e2c_eq_msg, eq_constraints], dim=-1))
            ...
            var_edges = var_input[batch_holder.vars_edge_graph.indices()[0, :]]
            const_edges = const_input[batch_holder.const_edge_graph.indices()[0, :]]
            edges = self.edge_update(torch.cat([var_edges, edges, edge_values, const_edges], dim=-1))

            eq_var_edges = var_input[batch_holder.eq_vars_edge_graph.indices()[0, :]]
            eq_const_edges = eq_const_input[batch_holder.eq_const_edge_graph.indices()[0, :]]
            eq_edges = self.eq_edge_update(torch.cat([eq_var_edges, eq_edges, eq_edge_values, eq_const_edges], dim=-1))
            ...
            #
            # var_msg = torch.cat(
            #     [variables, const2var_msg_pos, const2var_msg_neg, eq_const2var_msg, obj_multipliers, int_mask], dim=-1)
